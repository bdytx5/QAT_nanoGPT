...
def llama3_mlp(dim: int, hidden_dim: int, quantize_base: bool = False ) -> FeedForward:
    ...
    print(type(gate_proj), type(up_proj))
    print("#############"*10)
...
#         self.vocab_size = model.output.out_features  # assumes output is Linear(embed_dim, vocab_size)

#     def forward(self, idx, targets=None):
#         b, t = idx.size()
#         assert t <= self.block_size, f"Cannot forward sequence of length {t}, block size is only {self.block_size}"

#         x = self.model.tok_embeddings(idx)
...
#         optimizer = torch.optim.AdamW(optim_groups, lr=learning_rate, betas=betas, fused=fused) if fused \
#             else torch.optim.AdamW(optim_groups, lr=learning_rate, betas=betas)
